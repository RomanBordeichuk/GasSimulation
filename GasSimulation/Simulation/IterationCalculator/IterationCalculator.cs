...
namespace GasSimulation.Simulation.IterationCalculator
{
    ...
    {
        public static void Calculate(AllStates allStates)
        {
            double remT = 1;
            ...
            while (true)
            {
                CollisionState<AtomState, AtomState> atomCollision = 
                    ClosestAtomCollisionHelper.Calculate(ref allStates, remT);

                CollisionState<AtomState, RectState> rectCollision = 
                    ClosestRectCollisionHelper.Calculate(ref allStates, remT);

                if (MathHelper.Equals(atomCollision.T, -1) && MathHelper.Equals(rectCollision.T, -1))
                {
                ...
                if (MathHelper.Equals(rectCollision.T, -1) || 
                    (!MathHelper.Equals(atomCollision.T, -1) && atomCollision.T < rectCollision.T))
                {
                    MoveAll(allStates.Atoms, atomCollision.T);

                    (VelocityState v1, VelocityState v2) = CollisionCalculator.CalculateVelocities(
                        atomCollision.Obj1, atomCollision.Obj2, atomCollision.Angle);
